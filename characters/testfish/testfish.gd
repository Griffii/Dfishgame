...
func enter():
	...
	if player.sprite:
		if player.sprite.has_animation("swimming"):
			print("Playing swimming animation")
			player.sprite.play("swimming")  # Play swimming animation
		else:
			print("No 'swimming' animation found in player sprite.")
	else:
		print("Player sprite not found!")

	# Slow down velocity when entering water
	player.velocity = player.velocity * 0.8  

func exit():
	# Stop swimming animation when leaving water
	if player.sprite:
		player.sprite.stop()
	else:
		print("Player sprite not found on exit.")
